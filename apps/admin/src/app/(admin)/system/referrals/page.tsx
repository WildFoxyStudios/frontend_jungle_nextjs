...
export default function ReferralsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => adminApi.getReferrals({ page: String(page) }),
  });

  const columns: ColumnDef<{ id: number } & Record<string, unknown>>[] = [
    { accessorKey: "id", header: "ID", size: 60 },
    { accessorKey: "referrer", header: "Referrer", cell: ({ row }) => (row.original.referrer as { username?: string })?.username ?? "—" },
    { accessorKey: "referred", header: "Referred User", cell: ({ row }) => (row.original.referred as { username?: string })?.username ?? "—" },
  ...
  return (
    <AdminPageShell title="Referrals">
      <DataTable
