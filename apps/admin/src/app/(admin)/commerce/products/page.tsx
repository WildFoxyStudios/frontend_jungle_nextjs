...
export default function ProductsPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => adminApi.getAdminProducts({ page: String(page) }),
  });

  const columns: ColumnDef<{ id: number } & Record<string, unknown>>[] = [
    { accessorKey: "id", header: "ID", size: 60 },
    { accessorKey: "title", header: "Title" },
    { accessorKey: "price", header: "Price", cell: ({ row }) => `${row.original.currency ?? "USD"} ${row.original.price}` },
    ...
    { accessorKey: "created_at", header: "Date", cell: ({ row }) => new Date(row.original.created_at as string).toLocaleDateString() },
    {
      ...
      cell: ({ row }) => (
        <Button variant="destructive" size="sm" onClick={async () => {
          await adminApi.deleteAdminProduct(row.original.id as number);
