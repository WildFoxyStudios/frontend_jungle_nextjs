...
export default function PagesFieldsPage() {
  const qc = useQueryClient();
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ name: "", field_type: "text", required: false, placement: "page" });

  const { data, isLoading } = useQuery({ queryKey: ["admin", "profile-fields", "page"], queryFn: () => adminApi.getProfileFields() });

  const createMutation = useMutation({
    mutationFn: () => adminApi.createProfileField(form),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin", "profile-fields", "page"] }); setOpen(false); toast.success("Field created"); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => adminApi.deleteProfileField(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin", "profile-fields", "page"] }); toast.success("Deleted"); },
  });

  const fields = ((data ?? []) as (Field & { placement?: string })[]).filter((f) => f.placement === "page");

  return (
    <AdminPageShell title="Pages Custom Fields" description="Custom fields shown on page profiles" actions={
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>New Page Field</DialogTitle></DialogHeader>
          <div className="space-y-3 py-2">
            <div className="space-y-1"><Label>Field Name</Label><Input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} /></div>
            <div className="space-y-1"><Label>Field Type</Label><Select value={form.field_type} onValueChange={(v) => setForm((f) => ({ ...f, field_type: v }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["text", "url", "email", "phone", "date", "select", "textarea"].map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent></Select></div>
