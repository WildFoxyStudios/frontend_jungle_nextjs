...
import { Button, Input, Badge, Skeleton, Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter, Label } from "@jungle/ui";
    ...
    <AdminPageShell title="Currencies" actions={
      ...
      {isLoading ? <Skeleton className="h-48 w-full" /> : (
        <div className="border bg-card divide-y-2 divide-foreground shadow-sm">
          {currencies.length === 0 && <p className="text-center text-muted-foreground py-8 text-sm font-bold uppercase tracking-wide">No currencies yet.</p>}
          {currencies.map((c) => (
