...
import type { Job, JobQuestion } from "@jungle/api-client";
...
`;
 ...
 <div className="space-y-1.5">
 <Label htmlFor="app-location" className="flex items-center gap-1.5">
 ...
 <div className="space-y-1.5">
 <Label htmlFor="app-start-year">From</Label>
 <select id="app-start-year" className="h-9 w-full border bg-background px-3 text-sm" value={startDate} onChange={(e) => setStartDate(e.target.value)}>
 {years.map(y => <option key={y} value={y}>{y}</option>)}
 </select>
 </div>
 <div className="space-y-1.5">
 <Label htmlFor="app-end-year">To</Label>
 <select id="app-end-year" className="h-9 w-full border bg-background px-3 text-sm disabled:opacity-50" value={endDate} onChange={(e) => setEndDate(e.target.value)} disabled={currentlyWork}>
 {years.map(y => <option key={y} value={y}>{y}</option>)}
 </select>
 </div>
 </div>
 ...
 <div className="space-y-4">
 <div className="text-sm font-semibold text-muted-foreground">Requirements</div>
 {job.questions.map((q) => (
 <QuestionInput
 key={q.id}
 question={q}
...
/**
 * Plan §3.15 JA1 — renders the right input widget per `question_type`.
 * Falls back to `<Textarea>` for legacy rows without an explicit type.
 */
