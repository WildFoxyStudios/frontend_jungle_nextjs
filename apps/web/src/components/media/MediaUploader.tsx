...
import { Progress } from "@jungle/ui";
...
export function MediaUploader({ preset, onUpload, maxFiles = 10 }: MediaUploaderProps) {
 ...
 return (
 <div
 className={`border border-dashed border-border p-6 text-center transition-colors ${
 isDragging ? "bg-primary/10" : "hover:bg-secondary/40"
 }`}
 ...
 <button type="button" onClick={() => inputRef.current?.click()} className="text-primary hover:underline">browse</button>
 </p>
 {isUploading && (
