...
export function AgoraBroadcaster({
 ...
 setCamOn(true);
 onStart?.();
 toast.success(t("hostLiveSuccess"));
 } catch (err) {
 const msg =
 err instanceof Error ? err.message : typeof err === "string" ? err : t("startError");
 ...
 }, [channelName, callId, isConnecting, isLive, onStart, onError, cleanup, t]);
 ...
 return (
 <div className={`relative overflow-hidden border bg-black ${className}`}>
 <div
 ...
 <Radio className="h-12 w-12 text-white/50" aria-hidden />
 <p className="text-center text-sm text-white/80">{t("agoraReadyTitle")}</p>
 <code className="text-xs text-white/60 bg-white/10 px-2 py-1 rounded">{channelName}</code>
 <Button size="lg" onClick={start}>
 <Radio className="h-4 w-4 mr-2" aria-hidden /> {t("goLive")}
 </Button>
 </div>
 )}
 ...
 <p className="text-sm text-white/80">{t("connecting")}</p>
 </div>
 )}

 {isLive && (
 <>
 <div className="absolute left-3 top-3 flex items-center gap-1.5 border-2 border-white/80 bg-red-600 px-2 py-1 text-xs font-extrabold text-white">
