...
const PROVIDERS: Provider[] = [
 ...
 color: "#FF0000",
 embedFn: (url) => {
 const m = url.match(/(?:youtu\.be\/|(?:music\.|m\.|www\.)?youtube\.com\/(?:watch\?v=|embed\/|shorts\/|live\/))([A-Za-z0-9_-]{11})/);
 ...
 if (m[1]) return `https://player.twitch.tv/?video=${m[1]}&parent=${parent}&autoplay=false`;
 return `https://player.twitch.tv/?channel=${m[2]}&parent=${parent}&autoplay=false`;
 },
 },
 {
 name: "TikTok",
...
export function GenericOEmbedRenderer({ url, title, thumbnail }: GenericOEmbedRendererProps) {
 ...
 if (playing) {
 return (
 <div className={`w-full overflow-hidden border ${aspectClass}`}>
 <iframe
 src={embedUrl}
 ...
 <button
 type="button"
 className={`group relative cursor-pointer overflow-hidden border rounded-md bg-muted ${aspectClass}`}
 onClick={() => setPlaying(true)}
 aria-label={`Play embedded content from ${provider.name}`}
 >
 {thumbnail ? (
 <Image
 src={thumbnail}
 ...
 {/* Provider badge */}
 <div
 className="absolute left-3 top-3 border-2 border-white/80 px-2 py-0.5 text-xs font-extrabold text-white"
 ...
 <div className="flex h-14 w-14 items-center justify-center rounded-full border bg-white/90 shadow-md transition-transform group-hover:scale-110">
