...
interface PostRendererProps {
 ...
 /** When true, marks the first image media as priority for LCP. */
 priority?: boolean;
}

export function PostRenderer({ post, onPhotoClick, priority = false }: PostRendererProps) {
 ...
 {post.post_type === "forum" && (post as Post & { forum_thread_info?: { id: number; title: string; content?: string; reply_count: number; view_count: number; forum_name?: string } }).forum_thread_info && (
 <ForumThreadEmbed
 ...
 {post.link_url && post.post_type !== "ad" && post.post_type !== "forum" && !post.blog_info && !post.product_info && (
 <RichUrlPreviewEmbed
 url={post.link_url}
