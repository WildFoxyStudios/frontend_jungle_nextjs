...
export function UserHoverCard({ username, children }: UserHoverCardProps) {
 ...
 <span className="font-semibold text-sm truncate max-w-[120px]">{profile.first_name} {profile.last_name}</span>
 ...
 {!isMe && isFollowing ? (
 <Badge variant="secondary" className="px-2 py-0 h-6 text-[13px] font-medium">{tn("friends")}</Badge>
 ) : null}
 </div>
 ...
 <Button size="sm" className="h-9 flex-1 rounded-full text-[13px] font-semibold" onClick={handleFollow}>
 {isFollowing ? tp("unfollow") : tp("follow")}
 </Button>
 <Button size="sm" variant="outline" className="h-9 flex-1 rounded-full text-[13px] font-semibold" asChild>
 <Link href={`/messages?userId=${profile.id}`}>{tp("message")}</Link>
 </Button>
 </div>
 )}
 </div>
