...
export function InviteFriendsDialog({ onInvite, children, title = "Invite Friends" }: InviteFriendsDialogProps) {
 ...
 type="button"
 onClick={() => toggle(f.id)}
 className={`flex w-full items-center gap-2 border border-transparent p-2 text-left transition-colors hover:border-foreground hover:bg-secondary/60 ${selected.has(f.id) ? "border-foreground bg-secondary/60" : ""}`}
 >
