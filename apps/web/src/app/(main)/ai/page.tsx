...
type AiBalance = { remaining: number; limit: number; plan: string; reset_at: string };

export default function AiWriterPage() {
 const [wordBalance, setWordBalance] = useState<AiBalance | null>(null);
 const [imageBalance, setImageBalance] = useState<AiBalance | null>(null);

 const loadWordBalance = useCallback(async () => {
 try {
 const b = await contentApi.aiGetWordBalance();
 setWordBalance(b);
 } catch {
 setWordBalance(null);
 }
 ...
 const loadImageBalance = useCallback(async () => {
 try {
 const b = await contentApi.aiGetImageBalance();
 setImageBalance(b);
 } catch {
 ...
 useEffect(() => {
 void loadWordBalance();
 void loadImageBalance();
 }, [loadWordBalance, loadImageBalance]);
 ...
 <GeneratePostCard onDone={() => void loadWordBalance()} />
 </TabsContent>
 <TabsContent value="blog" className="mt-4">
 <GenerateBlogCard onDone={() => void loadWordBalance()} />
 </TabsContent>
 <TabsContent value="image" className="mt-4">
 <GenerateImageCard onDone={() => void loadImageBalance()} />
 </TabsContent>
 </Tabs>
 </div>
 );
...
function BalanceCard({
 ...
 <span>
 <span className="font-semibold text-2xl">{remaining.toLocaleString()}</span>
 <span className="text-muted-foreground"> / {limit.toLocaleString()}</span>
 </span>
 {resetAt && (
 <span className="text-[13px] font-medium text-muted-foreground self-end">
...
function GeneratePostCard({ onDone }: { onDone: () => void }) {
 ...
 <SelectTrigger id="post-tone"><SelectValue /></SelectTrigger>
 <SelectContent>
...
function GenerateBlogCard({ onDone }: { onDone: () => void }) {
 ...
 <SelectTrigger id="blog-length"><SelectValue /></SelectTrigger>
 <SelectContent>
...
function GenerateImageCard({ onDone }: { onDone: () => void }) {
 ...
 <div className="grid grid-cols-3 gap-3">
 <div className="space-y-1.5">
 <Label htmlFor="img-count">Count</Label>
 <Select value={String(count)} onValueChange={(v) => setCount(Number(v))}>
 <SelectTrigger id="img-count"><SelectValue /></SelectTrigger>
 <SelectContent>
 {[1, 2, 3, 4].map((n) => (
 ...
 rel="noopener noreferrer"
                className="relative block aspect-square overflow-hidden transition hover:bg-muted/50"
 >
              <Image src={url} alt={prompt} fill unoptimized className="object-cover" />
 </a>
 ))}
 </div>
 )}
...
function ResultBlock({ text }: { text: string }) {
 ...
 return (
 <div className="relative space-y-3 bg-muted/40 p-4">
