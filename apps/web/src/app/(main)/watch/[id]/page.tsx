...
export default function MovieDetailPage({ params }: Props) {
 ...
 <h1 className="text-2xl font-bold sm:text-[28px] line-clamp-1">{movie.title}</h1>
 </div>

 {/* Video player */}
 <div className="relative aspect-video bg-black overflow-hidden border">
 {playing && movie.video_url ? (
 <video src={movie.video_url} className="w-full h-full" controls autoPlay />
 ) : (
 <>
 {movie.thumbnail && (
 <Image src={movie.thumbnail} alt={movie.title} fill priority unoptimized className="object-cover" />
 )}
 <div className="absolute inset-0 flex items-center justify-center bg-black/30">
 <Button
 size="lg"
 className="rounded-full h-16 w-16 p-0 gap-0 border border-white/70"
 ...
 {movie.created_at && (
 <p className="text-[13px] font-medium text-muted-foreground">Added {formatDate(movie.created_at)}</p>
 )}
 </div>
 ...
 <div className="space-y-3">
 {comments.length === 0 ? (
 <p className="text-[15px] font-semibold text-muted-foreground text-center py-2">No comments yet</p>
 ) : comments.map((c) => (
