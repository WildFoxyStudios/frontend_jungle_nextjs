...
import { getCtaLabel } from "@/lib/cta";
...
interface PageRating {
 ...
 comment?: string;
 created_at?: string;
 user?: Pick<PublicUser, "avatar" | "first_name"> | null;
}
...
export default function PageDetailPage({ params }: Props) {
 ...
 useEffect(() => {
 pagesApi.getPage(slug).then(setPage).catch((e: unknown) => { toast.error("Failed to load data"); console.error(e); });
 }, [slug]);
 ...
 .then((r) => setRatings(Array.isArray(r) ? (r as PageRating[]) : []))
 .catch((e: unknown) => { toast.error("Failed to load data"); console.error(e); });
 }
 ...
 }, [page, activeTab, posts.length, likers.length, ratings.length, offers.length, services.length]);
 ...
 try {
 // Dedicated user search endpoint returns a well-typed PublicUser[].
 const users = await usersApi.searchUsers(q, 10);
 setAdminSearchResults(users ?? []);
 } catch (err) { console.error("[PageDetail] admin search failed", err); }
 ...
 } catch (err) { console.error("[PageDetail] add admin failed", err); toast.error("Failed to add admin"); }
 };
 ...
 } catch (err) { console.error("[PageDetail] remove admin failed", err); toast.error("Failed to remove admin"); }
 };
 ...
 } catch (err) { console.error("[PageDetail] like/unlike failed", err); toast.error("Action failed"); }
 };
 ...
 // Refresh ratings
 pagesApi.getPageRatings(page.id).then((r) => setRatings(Array.isArray(r) ? (r as PageRating[]) : []));
 } catch (err) {
 console.error("[PageDetail] rate failed", err); toast.error("Failed to rate");
 }
 };
 ...
 const pagePostsLcpId = firstHeroImagePostId(posts);
 ...
 <Image src={page.cover} alt="" fill priority unoptimized className="object-cover" />
 </div>
 )}

 <div className="space-y-4 px-3 py-4 sm:px-4">
 <div className="flex items-start gap-4">
 <Avatar className="h-20 w-20 shrink-0 rounded-lg border-4 border-background -mt-10 relative z-10">
 ...
 <p className="font-medium text-muted-foreground">No posts published yet.</p>
 </div>
 ) : (
 ...
 priority={pagePostsLcpId !== null && post.id === pagePostsLcpId}
 />
 ))}
 </div>
 )}
 ...
 {likers.length === 0 ? (
 <div className="py-12 text-center">
 <p className="font-medium text-muted-foreground">No likes yet.</p>
 </div>
 ) : (
 <Card>
 ...
 <AvatarImage src={resolveAvatarUrl(liker.avatar)} />
 <AvatarFallback>{liker.first_name?.[0] ?? "?"}</AvatarFallback>
 </Avatar>
 <div className="min-w-0">
 <p className="truncate text-sm font-medium">{liker.first_name} {liker.last_name}</p>
 ...
 <div className="bg-muted/40 p-5 space-y-4">
 <p className="text-[13px] font-medium text-muted-foreground">Rate & Review</p>
 <div className="flex items-center gap-2">
 {[1, 2, 3, 4, 5].map((star) => (
 <button
 key={star}
 ...
 className={`group transition-all hover:scale-110 active:scale-95 ${star <= myRating ? "text-yellow-500" : "text-muted-foreground"
 }`}
 >
 ...
 } catch (err) { console.error("[PageDetail] add service failed", err); toast.error("Failed to add service"); }
 }}
 >
 Add Service
 </Button>
 ...
 <p className="font-medium text-muted-foreground">No services listed yet.</p>
 </div>
 ) : (
 ...
 {svc.image ? (
 <Image src={svc.image} alt="" fill unoptimized className="object-cover" />
 ) : (
 ...
 } catch (err) { console.error("[PageDetail] delete service failed", err); toast.error("Failed to delete"); }
 }}
 ...
 <input
 className="w-full border px-3 py-1.5 text-sm bg-background"
 placeholder="Offer title"
 value={offerForm.title}
 onChange={(e) => setOfferForm((f) => ({ ...f, title: e.target.value }))}
 />
 <textarea
 className="w-full border px-3 py-1.5 text-sm bg-background resize-none"
 rows={2}
 placeholder="Description"
 ...
 } catch (err) { console.error("[PageDetail] create offer failed", err); toast.error("Failed to create offer"); }
 }}
 >
 Create
 </Button>
 ...
 <div className="py-12 text-center">
 <p className="font-medium text-muted-foreground">No offers yet.</p>
 </div>
 ) : (
 ...
 <div className="border bg-primary text-primary-foreground p-2 shrink-0">
 ...
 } catch (err) { console.error("[PageDetail] delete offer failed", err); toast.error("Failed to delete"); }
 }}
 >
 ...
 <div className="max-h-48 divide-y-2 divide-foreground overflow-y-auto border">
