...
export default function BroadcastsPage() {
 ...
 {broadcasts.map((b) => {
 const active = selectedBroadcast?.id === b.id;
 return (
 <Card
 key={b.id}
 ...
 <div className="flex h-10 w-10 shrink-0 items-center justify-center border bg-primary text-primary-foreground">
 <Megaphone className="h-5 w-5" />
 </div>
 <div className="min-w-0 flex-1">
 <p className="truncate text-sm font-semibold">
 {b.name ?? "Broadcast List"}
 </p>
 ...
 <>
 <div>
 <h3 className="text-base font-semibold">{selectedBroadcast.name ?? "Broadcast List"}</h3>
 <p className="text-[13px] font-medium text-muted-foreground">
 {selectedBroadcast.members.length} recipients
 </p>
 </div>

 <div className="flex flex-wrap gap-1">
 {selectedBroadcast.members
 ...
 <Badge variant="secondary" className="text-[10px]">+{selectedBroadcast.members.length - 8}</Badge>
 )}
 </div>
 ...
 disabled={sending}
 />
 <Button onClick={handleSendBroadcast} disabled={sending || !message.trim()} className="shrink-0 gap-1.5">
 ...
 Select a broadcast list to send a message.
 </p>
 )}
 </CardContent>
 </Card>
