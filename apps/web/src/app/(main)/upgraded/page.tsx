...
export default function UpgradedPage() {
 ...
 useEffect(() => {
 Promise.all([
 usersApi.getMe().catch((err) => { console.error("[UpgradedPage] getMe failed", err); return null; }),
 paymentsApi.getProPlans().catch((err) => { console.error("[UpgradedPage] getProPlans failed", err); return []; }),
 ]).then(([user, plans]) => {
