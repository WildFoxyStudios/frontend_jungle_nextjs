...
import { Card, CardContent, CardHeader, CardTitle, Button, Input, Label, Badge } from "@jungle/ui";
...
export default function SecuritySettingsPage() {
 ...
 <img src={qrCode} alt="2FA QR Code" className="w-48 h-48 border" />
 </div>
 )}

 {backupCodes.length > 0 && (
 <div className="space-y-2">
 <p className="text-sm font-semibold">Backup codes (save these somewhere safe):</p>
 <div className="grid grid-cols-2 gap-2">
 {backupCodes.map((code) => (
 <code key={code} className="text-xs bg-secondary/40 border px-2 py-1 font-mono">{code}</code>
 ))}
 </div>
 </div>
 )}
