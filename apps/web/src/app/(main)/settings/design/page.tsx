...
import { Button, Card, CardContent, CardHeader, CardTitle, Label } from "@jungle/ui";
...
export default function DesignSettingsPage() {
 ...
 { key: "dark", label: "Dark", icon: Moon, preview: "bg-zinc-900" },
 { key: "system", label: "System", icon: Monitor, preview: "bg-gradient-to-r from-white to-zinc-900" },
 ].map(({ key, label, icon: Icon, preview }) => (
 <button
 key={key}
 onClick={() => setTheme(key)}
				aria-pressed={theme === key}
 className={`relative flex flex-col items-center gap-2 p-4 border transition-transform ${
 theme === key
 ? "bg-primary text-primary-foreground"
 : "bg-card hover:bg-muted/50"
 }`}
 >
 <div className={`w-full h-16 border border-foreground ${preview}`} />
 <div className="flex items-center gap-1.5">
 <Icon className="h-4 w-4" />
 <span className="text-sm font-semibold">{label}</span>
 </div>
 {theme === key && <Check className="absolute top-2 right-2 h-4 w-4" />}
 </button>
 ))}
 </div>
 </CardContent>
 ...
 title={color.name}
 >
 <div className={`w-8 h-8 rounded-full ${color.bg} flex items-center justify-center border ${
 accentColor === color.value ? "border-foreground" : "border-transparent"
 }`}>
 {accentColor === color.value && <Check className="h-4 w-4 text-white" />}
 </div>
 <span className="text-[13px] font-medium text-muted-foreground">{color.name}</span>
 </button>
 ))}
 </div>
 </CardContent>
 ...
 <div>
 <Label className="text-sm font-semibold">Compact Mode</Label>
 <p className="text-[13px] font-medium text-muted-foreground">Reduce spacing between elements for a denser layout.</p>
 </div>
 <Button
