...
import { 
 Skeleton, Card, CardContent, Avatar, AvatarImage, AvatarFallback, Button
} from "@jungle/ui";
...
export function PokesClient() {
 ...
 <div className="h-10 w-10 border bg-primary/10 text-primary flex items-center justify-center">
 <Hand className="h-6 w-6" />
 </div>
 <div>
 <h1 className="text-2xl font-bold sm:text-[28px]">Pokes</h1>
 ...
 <div className="border border-dashed bg-card p-12 text-center space-y-2">
 <Hand className="h-12 w-12 mx-auto opacity-10" />
 <p className="font-medium text-muted-foreground">No new pokes. Why not poke a friend?</p>
 </div>
 ) : (
 pokes.map((item, i) => (
 <Card key={i} className="transition hover:bg-muted/50">
 ...
 <p className="font-semibold text-sm truncate">{item.user.first_name} {item.user.last_name}</p>
