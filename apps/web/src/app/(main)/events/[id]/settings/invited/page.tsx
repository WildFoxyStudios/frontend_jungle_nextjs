...
import { eventsApi, usersApi } from "@jungle/api-client";
...
export default function EventInvited({ params }: Props) {
 ...
 setSearching(true);
 try {
 // Use the dedicated user search endpoint: `PublicUser[]` directly.
 const users = await usersApi.searchUsers(query, 5);
 setResults(users ?? []);
 } catch {
