...
export default function EventInterested({ params }: Props) {
 const { id } = use(params);
 ...
 const loadInterested = useCallback(async (nextCursor?: string) => {
 try {
 const res = await eventsApi.getInterested(eventId, nextCursor);
 const data = Array.isArray(res?.data) ? res.data : [];
 if (nextCursor) {
 setInterested((prev) => [...prev, ...data]);
 } else {
 setInterested(data);
 }
 ...
 toast.error("Failed to load interested");
 } finally {
 setLoading(false);
 ...
 <h2 className="text-lg font-semibold">Interested ({interested.length})</h2>
 {interested.length === 0 ? (
 <p className="text-muted-foreground">No one is interested yet.</p>
 ) : (
 <>
