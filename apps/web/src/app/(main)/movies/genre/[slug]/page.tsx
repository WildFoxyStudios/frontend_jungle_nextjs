"use client";

import { use, useCallback, useEffect, useState } from "react";
import Image from "next/image";
import { contentApi } from "@jungle/api-client";
import type { Movie } from "@jungle/api-client";
import { Skeleton, Card, CardContent, Badge, Button } from "@jungle/ui";
import { toast } from "sonner";
import Link from "next/link";
import { Film, ArrowLeft } from "lucide-react";

interface Props { params: Promise<{ slug: string }> }

export default function MoviesByGenrePage({ params }: Props) {
 const { slug } = use(params);
 const [movies, setMovies] = useState<Movie[]>([]);
 const [loading, setLoading] = useState(true);
 const [cursor, setCursor] = useState<string | undefined>();
 const [hasMore, setHasMore] = useState(false);
 const [loadingMore, setLoadingMore] = useState(false);

 const loadMovies = useCallback(async (cur?: string) => {
 if (cur) setLoadingMore(true);
 try {
 const res = await contentApi.getMoviesByGenre(slug, cur);
 if (cur) {
 setMovies((prev) => [...prev, ...(res.data as Movie[])]);
 } else {
 setMovies(res.data as Movie[]);
 }
 setCursor(res.meta.has_more ? res.meta.cursor : undefined);
 setHasMore(res.meta.has_more);
 } catch (err) {
 toast.error(err instanceof Error ? err.message : "Failed to load movies");
 } finally {
 setLoading(false);
 setLoadingMore(false);
 }
 }, [slug]);

 useEffect(() => { void loadMovies(); }, [loadMovies]);

 const genreName = slug.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());

 return (
 <div className="max-w-5xl mx-auto px-4 py-6 space-y-4">
 <div className="flex items-center gap-3">
 <Link href="/watch" className="text-muted-foreground hover:text-foreground">
 <ArrowLeft className="h-5 w-5" />
 </Link>
 <Film className="h-6 w-6" />
 <h1 className="text-2xl font-bold">{genreName}</h1>
 <span className="text-muted-foreground text-sm">genre</span>
 </div>

 {loading ? (
 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
 {Array.from({ length: 8 }).map((_, i) => (
 <Skeleton key={i} className="aspect-video" />
 ))}
 </div>
 ) : movies.length === 0 ? (
 <div className="text-center py-16 text-muted-foreground">
 No movies found for genre &ldquo;{genreName}&rdquo;.
 </div>
 ) : (
 <>
 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
 {movies.map((movie) => (
 <Link key={movie.id} href={`/watch/${movie.id}`}>
 <Card className="cursor-pointer overflow-hidden border transition-colors hover:bg-muted/50">
 <div className="relative aspect-video bg-muted">
 {movie.thumbnail && (
 <Image
 src={movie.thumbnail}
 alt={movie.title}
 fill
 unoptimized
 className="object-cover"
 />
 )}
 </div>
 <CardContent className="p-3 space-y-1">
 <p className="font-semibold text-sm line-clamp-1">{movie.title}</p>
 {movie.release_year && (
 <Badge variant="secondary" className="text-xs">{movie.release_year}</Badge>
 )}
 <p className="text-xs text-muted-foreground">{movie.view_count} views</p>
 </CardContent>
 </Card>
 </Link>
 ))}
 </div>

 {hasMore && (
 <div className="text-center pt-2">
 <Button variant="outline" disabled={loadingMore} onClick={() => void loadMovies(cursor)}>
 {loadingMore ? "Loading…" : "Load more"}
 </Button>
 </div>
 )}
 </>
 )}
 </div>
 );
}
