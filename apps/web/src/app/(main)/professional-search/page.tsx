...
export default function ProfessionalSearchPage() {
 ...
 setLoading(true);
 try {
 // Professional search narrows to users with working/school/about fields.
 const res = await usersApi.searchProfessionals(query, location || undefined, 20);
 // The professional result type is a superset of PublicUser, so it's
 // compatible with the UI which only reads a handful of profile fields.
 setUsers((res ?? []) as unknown as ProfessionalUser[]);
 } catch {
 ...
 <h1 className="text-2xl font-bold sm:text-[28px]">Professional Search</h1>
 </div>
 ...
 <p className="font-medium text-muted-foreground">No professionals found matching your criteria.</p>
 </div>
 ) : (
 filteredUsers.map((user) => (
 <Card key={user.id} className="transition hover:bg-muted/50">
