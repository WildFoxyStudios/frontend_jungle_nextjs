...
export default function NearbyShopsPage() {
 ...
 <Card className="cursor-pointer overflow-hidden transition hover:bg-muted/50">
 <div className="relative aspect-square bg-muted">
 {product.images?.[0] && (
 <Image src={product.images[0].url} alt={product.title} fill unoptimized className="object-cover" />
 )}
 </div>
 <CardContent className="p-3">
 <p className="font-medium text-sm truncate">{product.title}</p>
 <p className="text-sm font-semibold text-primary">{product.currency} {product.price}</p>
 {product.location && (
