...
import { ThumbsUp, Quote, Share2 } from "lucide-react";
...
export default function ForumThreadPage({ params }: Props) {
 ...
 setSharing(true);
 try {
 await contentApi.shareForumThread(threadId);
 toast.success("Shared on your timeline");
 } catch {
 ...
 <h1 className="text-2xl font-bold sm:text-[28px] mb-2">{thread.title}</h1>
 <Button
 type="button"
 variant="ghost"
 size="sm"
 ...
 <div className="border-l-4 border-foreground pl-3 mb-2 text-sm text-muted-foreground italic">
 ...
 {quotedReply && (
 <div className="border-l-4 border-primary pl-3 text-sm text-muted-foreground bg-secondary/40 p-2">
 <p className="text-[13px] font-medium mb-1">Quoting reply:</p>
