...
function accumulatePages(pages) {
...
describe("PBT: Feed pagination — preserves order and uniqueness", () => {
    test("concatenated pages have no duplicate IDs and preserve server order (100 runs)", () => {
        fc.assert(fc.property(fc.array(fc.array(fc.record({ id: fc.integer({ min: 1, max: 10_000 }) }), { minLength: 5, maxLength: 20 }), { minLength: 2, maxLength: 10 }), (rawPages) => {
            // Ensure globally unique IDs
            let counter = 1;
            const uniquePages = rawPages.map((page) => page.map((post) => ({ ...post, id: counter++ })));
...
//# sourceMappingURL=feed.property.test.js.map